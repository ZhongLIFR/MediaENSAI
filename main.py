...
if __name__ == '__main__':
    ...
    for file in files:  # 遍历文件夹
        if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
            f = path + "/" + file;  # 打开文件
            print(f)
            userinput = input("Please add this song into Database:")
            songs.append(read(userinput))
            name = userinput.split(separator, 1)[0]
    ...
    print('Sample: Please enter an audio sample file to identify: ')
    #raw_input() was renamed to input()
    userinput = input('---> ')
    sample = read(userinput)
    userinput = userinput.split(separator, 1)[0]

    print('Sample: Analyzing the audio sample: ' + str(userinput))
    ...
    print('Sample: The size of the spectrogram is time: ' + str(time) + ' and freq: ' + str(freq))

    threshold = pp.find_thres(spectrogram, percentile, base)

    peaks = pp.peak_pick(spectrogram, f_dim1, t_dim1, f_dim2, t_dim2, threshold, base)

    print('Sample: The initial number of peaks is:' + str(len(peaks)))
    peaks = pp.reduce_peaks(peaks, fftsize, high_peak_threshold, low_peak_threshold)
    print('Sample: The reduced number of peaks is:' + str(len(peaks)))
    ...
    print('Sample: The dimensions of the hash matrix of the sample: ' + str(hashSample.shape))

    print('Sample: Attempting to identify the sample audio clip.')
    ...
    for i in timepairs:
        ...
        songbins[int(i[2])] += 1
    # Identify the song
    print('Sample: The sample song is: ' + str(songnames[np.argmax(songbins)]))
